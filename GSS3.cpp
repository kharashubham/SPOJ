...
void update_query(int node, int start, int end, int idx, int val)
{
	if (end == start+1)
	{
		a[idx] = val;
		sum[node].total = sum[node].pre = sum[node].suf = sum[node].inv = val;
	}
	else
	{
		int mid = start + (end - start + 1)/2;
		if (idx < mid)
			update_query(2*node, start, mid, idx, val);
		else
			update_query(2*node+1, mid, end, idx, val);
